...
public class BTGpsLocationProvider implements LocationProviderInterface {
    ...
    private void reportLocation(Location loc, boolean isValid) {
        ...
        synchronized (mLocation) {
            ...
            if (D) {
                Log.d(TAG,
                        "reportLocation lat: " + mLocation.getLatitude() + " long: "
     ...
     * report sats status
     */
    private void reportSvStatus(int svCount, int mSvs[], float mSnrs[], float mSvElevations[],
            float mSvAzimuths[], int mSvMasks[]) {
    ...
    private void handleNMEAMessages(String sentences) {
        ...
        if (btsvc.getRefreshRate() != newRate) {
            if (D)
                Log.d(TAG,
                        "Setting refresh rate to: " + newRate + " was: " + btsvc.getRefreshRate());
